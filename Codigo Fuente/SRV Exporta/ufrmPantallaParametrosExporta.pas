...
uses
  ...
  dxSkinValentine, dxSkinXmas2008Blue, LibraryIngresos_Intf, dxSkinDarkSide,
...
type
  TfrmPantallaParametrosExporta = class(TdxCustomRibbonForm)
...
function ObtenParametros(Parametros, Titulo: String): TParametrosExporta;
var
  Aux: TfrmPantallaParametrosExporta;
begin
  Result:=nil;
  Aux:=TfrmPantallaParametrosExporta.Create(nil);
  Aux.Caption:=Titulo;
...
procedure TfrmPantallaParametrosExporta.PrenderParametros(Cad: String);
var
  i, j : Integer;
...
begin
  ...
  cdsEstaciones.Open;
  cbEstacionIni.EditValue:=cdsEstaciones.FieldByName('NUMEROESTACION').AsInteger;
  cbEstacionFin.EditValue:=cdsEstaciones.FieldByName('NUMEROESTACION').AsInteger;
end;

procedure TfrmPantallaParametrosExporta.btnCancelarClick(Sender: TObject);
begin
  ModalResult:=mrCancel;
end;

procedure TfrmPantallaParametrosExporta.cbEstacionIniExit(Sender: TObject);
begin
  cbEstacionFin.EditValue:=cbEstacionIni.EditValue;
end;

procedure TfrmPantallaParametrosExporta.FormCreate(Sender: TObject);
begin
  Width:=600;
  Height:=500;
...
procedure TfrmPantallaParametrosExporta.btnAceptarClick(Sender: TObject);
begin
...
procedure TfrmPantallaParametrosExporta.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
