...
begin
  ...
  if (iSource is LibraryIngresos_Intf.TParametrosBI) then begin
    ...
    Self.Alumno := lSource.Alumno;
    Self.AlumnoIni := lSource.AlumnoIni;
    Self.AlumnoFin := lSource.AlumnoFin;
    Self.Grupo := lSource.Grupo;
    Self.Ciclo := lSource.Ciclo;
    Self.Maestro := lSource.Maestro;
    Self.MaestroIni := lSource.MaestroIni;
    Self.MaestroFin := lSource.MaestroFin;
    Self.Nivel := lSource.Nivel;
    Self.TipoPago := lSource.TipoPago;
    Self.TipoCalificacion := lSource.TipoCalificacion;
    Self.Usuario := lSource.Usuario;
    Self.Familia := lSource.Familia;
...
begin
  if TROSerializer(ASerializer).RecordStrictOrder then begin
    ...
    l_Alumno := Self.Alumno;
    try
      TROSerializer(ASerializer).ReadInteger('Alumno', TypInfo.otSLong, l_Alumno);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'Alumno']));
    end;
    Self.Alumno := l_Alumno;
    l_AlumnoIni := Self.AlumnoIni;
    try
      TROSerializer(ASerializer).ReadInteger('AlumnoIni', TypInfo.otSLong, l_AlumnoIni);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'AlumnoIni']));
    end;
    Self.AlumnoIni := l_AlumnoIni;
    l_AlumnoFin := Self.AlumnoFin;
    try
      TROSerializer(ASerializer).ReadInteger('AlumnoFin', TypInfo.otSLong, l_AlumnoFin);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'AlumnoFin']));
    end;
    Self.AlumnoFin := l_AlumnoFin;
    l_Grupo := Self.Grupo;
    try
      TROSerializer(ASerializer).ReadInteger('Grupo', TypInfo.otSLong, l_Grupo);
    except
      on E: SysUtils.Exception do
    ...
    l_Ciclo := Self.Ciclo;
    try
      TROSerializer(ASerializer).ReadInteger('Ciclo', TypInfo.otSLong, l_Ciclo);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'Ciclo']));
    end;
    Self.Ciclo := l_Ciclo;
    l_Maestro := Self.Maestro;
    try
      TROSerializer(ASerializer).ReadInteger('Maestro', TypInfo.otSLong, l_Maestro);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'Maestro']));
    end;
    Self.Maestro := l_Maestro;
    l_MaestroIni := Self.MaestroIni;
    try
      TROSerializer(ASerializer).ReadInteger('MaestroIni', TypInfo.otSLong, l_MaestroIni);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'MaestroIni']));
    end;
    Self.MaestroIni := l_MaestroIni;
    l_MaestroFin := Self.MaestroFin;
    try
      TROSerializer(ASerializer).ReadInteger('MaestroFin', TypInfo.otSLong, l_MaestroFin);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'MaestroFin']));
    end;
    Self.MaestroFin := l_MaestroFin;
    l_Nivel := Self.Nivel;
    try
      TROSerializer(ASerializer).ReadInteger('Nivel', TypInfo.otSLong, l_Nivel);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'Nivel']));
    end;
    Self.Nivel := l_Nivel;
    l_TipoPago := Self.TipoPago;
    try
      TROSerializer(ASerializer).ReadInteger('TipoPago', TypInfo.otSLong, l_TipoPago);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'TipoPago']));
    end;
    Self.TipoPago := l_TipoPago;
    l_TipoCalificacion := Self.TipoCalificacion;
    try
      TROSerializer(ASerializer).ReadInteger('TipoCalificacion', TypInfo.otSLong, l_TipoCalificacion);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'TipoCalificacion']));
    end;
    Self.TipoCalificacion := l_TipoCalificacion;
    ...
    l_Familia := Self.Familia;
    try
      TROSerializer(ASerializer).ReadInteger('Familia', TypInfo.otSLong, l_Familia);
    except
      on E: SysUtils.Exception do
        uROClasses.RaiseError(SysUtils.Format('Exception "%s" with message "%s" happens during reading field "%s".', [E.ClassName, E.Message, 'Familia']));
    end;
    Self.Familia := l_Familia;
    ...
    try
      TROSerializer(ASerializer).ReadInteger('Status', TypInfo.otSLong, l_Status);
    except
      on E: SysUtils.Exception do
...
begin
  if TROSerializer(ASerializer).RecordStrictOrder then begin
    ...
    l_Alumno := Self.Alumno;
    TROSerializer(ASerializer).WriteInteger('Alumno', TypInfo.otSLong, l_Alumno);
    l_AlumnoIni := Self.AlumnoIni;
    TROSerializer(ASerializer).WriteInteger('AlumnoIni', TypInfo.otSLong, l_AlumnoIni);
    l_AlumnoFin := Self.AlumnoFin;
    TROSerializer(ASerializer).WriteInteger('AlumnoFin', TypInfo.otSLong, l_AlumnoFin);
    l_Grupo := Self.Grupo;
    TROSerializer(ASerializer).WriteInteger('Grupo', TypInfo.otSLong, l_Grupo);
    l_Ciclo := Self.Ciclo;
    TROSerializer(ASerializer).WriteInteger('Ciclo', TypInfo.otSLong, l_Ciclo);
    l_Maestro := Self.Maestro;
    TROSerializer(ASerializer).WriteInteger('Maestro', TypInfo.otSLong, l_Maestro);
    l_MaestroIni := Self.MaestroIni;
    TROSerializer(ASerializer).WriteInteger('MaestroIni', TypInfo.otSLong, l_MaestroIni);
    l_MaestroFin := Self.MaestroFin;
    TROSerializer(ASerializer).WriteInteger('MaestroFin', TypInfo.otSLong, l_MaestroFin);
    l_Nivel := Self.Nivel;
    TROSerializer(ASerializer).WriteInteger('Nivel', TypInfo.otSLong, l_Nivel);
    l_TipoPago := Self.TipoPago;
    TROSerializer(ASerializer).WriteInteger('TipoPago', TypInfo.otSLong, l_TipoPago);
    l_TipoCalificacion := Self.TipoCalificacion;
    TROSerializer(ASerializer).WriteInteger('TipoCalificacion', TypInfo.otSLong, l_TipoCalificacion);
    l_Usuario := Self.Usuario;
    TROSerializer(ASerializer).WriteInteger('Usuario', TypInfo.otSLong, l_Usuario);
    l_Familia := Self.Familia;
    TROSerializer(ASerializer).WriteInteger('Familia', TypInfo.otSLong, l_Familia);
    l_Valor1 := Self.Valor1;
    TROSerializer(ASerializer).WriteInteger('Valor1', TypInfo.otSLong, l_Valor1);
    l_Status := Self.Status;
    TROSerializer(ASerializer).WriteInteger('Status', TypInfo.otSLong, l_Status);
    l_Serie := Self.Serie;
    TROSerializer(ASerializer).WriteAnsiString('Serie', l_Serie);
    l_Factura := Self.Factura;
...
function TServiceIngresos_Proxy.AbreTurno(const IDTURNO: Integer; const IDESTACION: Integer): AnsiString;
var
  lMessage: IROMessage;
  lTransportChannel: IROTransportChannel;
begin
...
procedure TServiceIngresos_Proxy.GuardarDatosFactura(const DatosFactura: TDatosFactura);
var
  lMessage: IROMessage;
  lTransportChannel: IROTransportChannel;
begin
