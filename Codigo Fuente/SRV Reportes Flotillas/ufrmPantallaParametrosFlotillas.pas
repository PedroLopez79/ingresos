...
uses uDMFlotillas, ufrmBuscarClienteCredito, ufrmBuscar, UtileriasComun;
...
begin
  ...
  begin
    Aux:= DMFlotillas.Server.DatosCliente(Cliente.EditValue);
    if Aux <> nil then
    begin
...
begin
  ...
  Parametros.Identificador:=DMFlotillas.Identificador;
  ...
  DMFlotillas.AgregarOpcionCombo(edtDiaFacturar.Properties.Items, 'TODOS', -1);
 // cdsDiaFacturar.ParamByName('Identificador').AsInteger:=DM.Identificador;
  cdsDiaFacturar.Open;
  while not cdsDiaFacturar.EOF do
  begin
    DMFlotillas.AgregarOpcionCombo(edtDiaFacturar.Properties.Items, cdsDiaFacturar.FieldByName('Nombre').AsString, cdsDiaFacturar.FieldByName('GrupoFacturarID').Value);
    cdsDiaFacturar.Next;
  end;    // while
  cdsDiaFacturar.Close;
  edtDiaFacturar.EditValue:=-1;

  DMFlotillas.AgregarOpcionCombo(edtFormaDeCompra.Properties.Items, 'TODAS', -1);
  cdsTipoTarjeta.Open;
  while not cdsTipoTarjeta.EOF do
  begin
    DMFlotillas.AgregarOpcionCombo(edtFormaDeCompra.Properties.Items, cdsTipoTarjeta.FieldByName('Nombre').AsString, cdsTipoTarjeta.FieldByName('FormaCompraID').Value);
    cdsTipoTarjeta.Next;
  end;    // while
  ...
  edtEjercicio.Value:=DMFlotillas.Ejercicio;
  cbPeriodoIni.EditValue:=DMFlotillas.Periodo;
  cbPeriodoFin.EditValue:=DMFlotillas.Periodo;

  cdsSerie.Open;
  while not cdsSerie.EOF do
  begin
    DMFlotillas.AgregarSerieCombo(edtSerie3.Properties.Items, cdsSerie.FieldByName('Serie').AsString, cdsSerie.FieldByName('Serie').AsString);
    DMFlotillas.AgregarSerieCombo(edtSerie.Properties.Items, cdsSerie.FieldByName('Serie').AsString, cdsSerie.FieldByName('Serie').AsString);
    cdsSerie.Next;
  end;
  cdsSerie.Close;
end;
