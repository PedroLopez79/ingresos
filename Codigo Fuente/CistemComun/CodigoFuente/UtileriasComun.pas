...
{$IFNDEF SINPASS} function Login(Validador: TFuncLogin): Integer; {$ENDIF}
...
function depuraCadena(Cadena: WideString):WideString;

implementation

uses
{$IFNDEF SINPASS}		ufrmPassword, {$ENDIF} Winsock;
...
begin
  ...
  if Track1 = '' then
    Track1:=Track2;
  if (Length(Track1) > 6) and (Copy(Track1, 1, 6) = '124671') then
  begin
    Delete(Track1, 1, 6);
  end;
  if (Length(Track2) > 6) and (Copy(Track2, 1, 6) = '124671') then
  begin
    Delete(Track2, 1, 6);
  end;
end;
      ...
      try
        ...
        try
          ...
          if FirstOctet in [1..4] then
            SockAddr.sin_addr := in_addr(inet_addr(PAnsiChar(GetRandomSimilarIP(HostIP))))
          else
...
begin
  ...
  begin
      if (Cadena[x] <> ' ') and (band = False) then
          Result:= Result + Cadena[x]
      else begin
           if Cadena[x] <> ' ' then begin Result:= Result + Cadena[x]; band:= false; end;
           if (Cadena[x] = ' ') and (band = false) then begin
               if Cadena[x+1] <> '|' then Result:= Result + Cadena[x];
               band:= True;
           end;
      end;
  end;
